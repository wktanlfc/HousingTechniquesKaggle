...
## Comparatively less outliers, cleaner for analysis.
```{r}
ggarrange(
...
fullset <- rbind(train.clean, test.clean) 
...
```
---
## Out of **195** variables, the best model would pick between 140 down to 70 variables.
```{r}
set.seed(466846)
fit3 <- cv.glmnet(y = y,  # Finish this (5 lines)
                  x = x,
...
xvals <- model.matrix(eq , data = fullset[fullset$Test == 1,] , na.action = na.pass)

lasso.pred <- predict(fit3, xvals, s = 'lambda.min',
  type = "response")
lasso.results <- lasso.pred %>% as.data.frame() %>% exp()
# res <- cbind(ids, lasso.results)
# Make predictions using testing data
#lasso.pred <- predict(fit3, xvals,  type ="response", s = "lambda.min")
```

# Model Performed Better, at **0.14** RMSE on Leaderboard.
```{r, eval= TRUE}
lasso.results <- lasso.pred %>% as.data.frame() %>% exp() 
lasso.results
```
